...
class ChordFeature(nn.Module):
    def __init__(self):
        ...
        self.chord_conv1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0),
                                     bias=False)
        self.chord_conv2 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0),
                                     bias=False)
    ...
    def forward(self, x):
        chord_x = x.view(-1, 1, 96 * 2, 12, 5)
        chord_x = torch.sum(chord_x, 4, keepdim=True)  # 1,192,12
        chord_x = chord_x.view(-1, 1, 96 * 2, 12)
...
class BarDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = self.sigmoid(self.linear(out))
